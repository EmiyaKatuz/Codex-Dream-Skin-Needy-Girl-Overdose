...
const composerToolbarSelector = ':is(.composer-surface-chrome [class*="_footer_"], [class*="_ComposerLayoutRoot_"] [class*="_ComposerLayoutFooter_"], [data-composer-surface-variant][data-composer-radius-variant] :is([data-composer-footer-responsive], [class*="_ComposerLayoutFooter_"], [class*="_footer_"]))';
  ...
  if (!settings && !settingsPanel && generic) {
    ...
    if (genericComposer) {
      partFixtures.composer = makeDomNode(
        ...
        modernComposerLayout ? [composerLayoutRootSelector, composerRootSelector] : [genericComposerOwnerSelector],
      );
      const inputParent = modernComposerLayout
        ? (partFixtures.composerFooter = makeDomNode(
          "generic-composer-footer", partFixtures.composer, new Map(), [genericComposerOwnerSelector, composerRejectSelector],
        ))
...
export async function runRendererRuntimeTest(assetRoot) {
  ...
  const sidebar = ':is\\((?:__DREAM_SELECTOR_LEFT_PANEL__|aside\\.app-shell-left-panel), \\[data-testid="app-shell-floating-left-panel"\\]\\)';
  ...
  assert.match(
    css,
    new RegExp(`${sidebar} :is\\(button, a\\):hover ${noInlineColor}\\s*\\{\\s*color:\\s*var\\(--ds-accent\\) !important;`),
