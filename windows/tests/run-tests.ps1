...
'@
  ...
  if ($idempotentTheme.Theme.id -cne 'preserved-theme' -or $afterReinitCount -ne 3) {
  ...
  # The preceding background-only contract intentionally keeps the active
  # theme ID. Make this fixture explicitly represent the legacy custom-theme
  # migration path before removing its schema marker.
  $legacyTheme.id = 'custom'
