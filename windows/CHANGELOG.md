...
- 同步上游 `34335d2`，保留超天酱主题、三端 Internet Angel 扩展和本仓库更新源。
- 兼容新版 Codex 输入框、建议卡、消息气泡和宽表格，排除 Pet 透明窗口，保留用户设置的字体与首页输入框圆角。
- Windows 默认使用受管 CDP profile；首次使用需要在该 profile 中登录，后续复用该目录。渲染验证增加横向溢出检查。
- macOS 重新应用主题时尊重退出取消，更新检查遇到 API 限流时回退到本仓库 Release 跳转地址，并从 App 包内读取当前版本。
- CDP 端点长时间不可用时延长 watcher 重试间隔，减少轮询和重复日志。
